...
// =========================================================
// 웹 검색 API — Google Custom Search (설정 시) or SearXNG public (무료)
// =========================================================
...
async function searchViaSearX(query, maxResults) {
  const errors = [];
  for (let i = 0; i < SEARX_INSTANCES.length; i++) {
    const base = SEARX_INSTANCES[(searxIdx + i) % SEARX_INSTANCES.length];
    const url = `${base}/search?q=${encodeURIComponent(query)}&format=json&categories=general&language=ko-KR`;
    try {
      const r = await fetch(url, {
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; MCLAW/1.0)', 'Accept': 'application/json' },
        signal: AbortSignal.timeout(6000),
      });
      if (!r.ok) { errors.push(`${base}: HTTP ${r.status}`); continue; }
      const data = await r.json();
      searxIdx = (searxIdx + i + 1) % SEARX_INSTANCES.length;
      const items = (data.results || []).slice(0, maxResults).map(item => ({
        type: 'web',
        title: (item.title || '').slice(0, 120),
        snippet: (item.content || item.url || '').slice(0, 400),
        url: item.url || '',
        source: (() => { try { return new URL(item.url).hostname; } catch(e) { return item.url; } })(),
      }));
      return { ok: true, items, engine: base };
    } catch (e) {
      errors.push(`${base}: ${e.message}`);
    }
  }
  throw new Error('모든 SearXNG 인스턴스 실패: ' + errors.slice(0, 2).join(' | '));
}

async function searchViaGoogle(query, maxResults) {
  const key = process.env.GOOGLE_CSE_KEY;
  const cx  = process.env.GOOGLE_CSE_ID;
  const url = `https://www.googleapis.com/customsearch/v1?key=${key}&cx=${cx}&q=${encodeURIComponent(query)}&num=${Math.min(maxResults, 10)}`;
  ...
  const items = (data.items || []).map(item => ({
    ...
    url: item.link || '',
    source: (() => { try { return new URL(item.link).hostname; } catch(e) { return item.link; } })(),
  }));
  return { ok: true, items, engine: 'Google CSE' };
}
...
app.post('/api/ws/search', toolAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (process.env.GOOGLE_CSE_KEY && process.env.GOOGLE_CSE_ID) {
      try { result = await searchViaGoogle(query, maxResults); }
      catch (e) { console.warn('Google CSE 실패, SearXNG fallback:', e.message); result = await searchViaSearX(query, maxResults); }
    } else {
      result = await searchViaSearX(query, maxResults);
    }
    if (!result.items || result.items.length === 0) {
      return res.json({ ok: true, query, results: [], totalFound: 0, message: `"${query}" 검색 결과 없음.` });
    }
    res.json({ ok: true, query, engine: result.engine, totalFound: result.items.length, results: result.items });
  } catch (e) {
